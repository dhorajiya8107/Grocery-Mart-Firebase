...
import { collection, getDocs, query, limit, orderBy, doc, setDoc } from "firebase/firestore"
...
const categories = [
  { name: "Grocery", image: Cat1, path: "Vegetables%20&%20Fruits" },
  { name: "Dairy & Bread", image: Cat2, path: "Dairy%20&%20Bread" },
  { name: "Tea, Coffee and More", image: Cat3, path: "Tea,%20Coffee%20&%20Health%20Drinks" },
  { name: "Atta, Rice and Dal", image: Cat4, path: "Atta,%20Rice%20&%20Dal" },
  { name: "Munchies", image: Cat5, path: "Munchies" },
  { name: "Sauces and Spreads", image: Cat6, path: "Sauces%20&%20Spreads" },
  { name: "Body Care", image: Cat7, path: "Body%20Care" },
  { name: "Ice Creams and More", image: Cat8, path: "Ice%20Creams%20&%20Frozen%20Desserts" },
  { name: "Cold Drinks", image: Cat9, path: "Cold%20Drinks" },
]
  ...
  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center bg-gradient-to-b from-gray-50 to-gray-100">
        <div className="h-16 w-16 border-4 border-gray-200 border-t-emerald-500 rounded-full animate-spin"></div>
      </div>
    )
  }
            ...
            {categories.map((category, index) => (
              <SwiperSlide key={index}>
                <div
                  ...
                  onClick={() => router.push(`/categories/${decodeURIComponent(category.path)}`)}
                >
                  ...
                  <p className="text-xs sm:text-sm font-medium text-gray-700 text-center line-clamp-2 group-hover:text-emerald-600">
                    {category.name}
                  </p>
                </div>
              </SwiperSlide>
